...
def extract_note_events_fast(track, note_range):
    note_events = defaultdict(list)
    current_time = 0
    
    note_set = set(note_range) if not isinstance(note_range, set) else note_range
    
    for msg in track:
        current_time += msg.time
        if msg.type in {'note_on', 'note_off'} and msg.note in note_set:
...
def extract_text_events_fast(track):
    text_events = []
    current_time = 0
    for msg in track:
        current_time += msg.time
...
def group_events_by_time_window_fast(events, time_window):
    ...
    for time in sorted_times:
        notes = events[time]
        found_group = False
        recent_groups = [t for t in grouped_events.keys() if abs(time - t) <= time_window]
        for group_time in recent_groups:
...
def compare_tracks_fast(track1_events, track2_events, time_window, time_threshold):
    differences = []
    grouped1 = group_events_by_time_window_fast(track1_events, time_window)
    grouped2 = group_events_by_time_window_fast(track2_events, time_window)
    ...
    for time in all_times:
        ...
        if events1 != events2:
            ...
            nearby_times = [t for t in all_times if abs(time - t) <= time_threshold and time != t]
            for other_time in nearby_times:
...
def compare_text_events_fast(text_events1, text_events2):
    diffs = []
...
def visualize_midi_changes_fast(differences, text_differences, note_name_map, track_name, output_folder, session_id):
    ...
    removed_plotted = False
    added_plotted = False
    
    for time, removed, added in differences:
        for note_val, _, _ in removed:
            label = 'Removed' if not removed_plotted else ""
            ax.scatter(time, y_labels[note_val], c='red', marker='s', s=100, edgecolors='black', label=label)
            removed_plotted = True
        for note_val, _, _ in added:
            label = 'Added' if not added_plotted else ""
            ax.scatter(time, y_labels[note_val], c='green', marker='s', s=100, edgecolors='black', label=label)
            added_plotted = True

    text_change_plotted = False
    if text_differences:
        if 'text' not in y_labels:
            y_labels['text'] = y_pos_counter
        for time, _, _ in text_differences:
            label = 'Text Change' if not text_change_plotted else ""
            ax.scatter(time, y_labels['text'], c='blue', marker='^', s=100, label=label)
            text_change_plotted = True
...
def run_comparison_fast(midi_file1_path, midi_file2_path, session_id, output_folder='out', format="json"):
    ...
    for track_name in tracks_to_actually_compare:
        ...
        text_events1 = extract_text_events_fast(track1) if track1 else []
        text_events2 = extract_text_events_fast(track2) if track2 else []

        note_diffs = compare_tracks_fast(note_events1, note_events2, TIME_WINDOW, TIME_THRESHOLD)
        text_diffs = compare_text_events_fast(text_events1, text_events2)
        ...
        if note_diffs or text_diffs:
            ...
            image_path = visualize_midi_changes_fast(note_diffs, text_diffs, note_map, track_name, output_folder, session_id)
            
            if image_path:
                generated_results.append((track_name, image_path))
                logging.info(f"Optimized comparison: Differences found in '{track_name}'. Image saved to {image_path}")
        else:
            logging.info(f"Optimized comparison: '{track_name}' has no significant changes.")
